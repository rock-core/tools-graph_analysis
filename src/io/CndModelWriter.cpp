...
void CndModelWriter::write(const std::string& filename, const BaseGraph::Ptr& graph)
{
    YAML::Node doc;
    int i = 1;
    ...
    while (vit->next())
    {
        ...
        task_graph::Task::Ptr task = dynamic_pointer_cast<task_graph::Task>(vit->current());
        // It is a task, so add the task to the list of tasks
        doc["tasks"][task->getLabel()]["type"] = task->getTemplateLabel();
        ...
        // FIXME: We dont know the concept of deployments yet, so we create 1 deployment/task
        // NOTE: This is a default deployment
        doc["deployments"][i]["deployer"] = "orogen";
        doc["deployments"][i]["hostID"] = "localhorst";
        std::string rubyShit = createRubyStuff(task->getTemplateLabel());
        doc["deployments"][i]["process_name"] = rubyShit;
        doc["deployments"][i]["taskList"][task->getLabel()] = rubyShit;
        doc["deployments"][i]["taskList"][task->getLabel()+"_Logger"] = rubyShit + "_Logger";
        i++;
    }
    ...
    while (eit->next())
    {
        ...
        if (eit->current()->getClassName() != task_graph::PortConnection::edgeType())
            continue;

        task_graph::PortConnection::Ptr conn = dynamic_pointer_cast<task_graph::PortConnection>(eit->current());
