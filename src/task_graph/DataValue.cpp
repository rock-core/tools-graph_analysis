...
DataValue::DataValue(const std::string& label)
    : Vertex(label)
{
}

Vertex::Ptr DataValue::getOwner(const BaseGraph::Ptr& graph) const
{
    Vertex::Ptr owner;
    ...
    while (eit->next())
    {
        ...
        if (owner)
            throw std::runtime_error("DataValue::getOwner: Multiple owners detected");
